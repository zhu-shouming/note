...
class TestWorkload:
    def setup_class(self):
        ...
        if get_value_from_json(r, "$.data.total") == 1:
            pass
        else:
    ...
    def test_create_workload(self, args):
        ...
        while cnt < 5:
            ...
            r_check = self.user.cce_client.get_deployment_by_ns(
                payload_type, settings['CLUSTER_ID'], ns, name=deploy_name
            )
            deploy = get_value_from_json(r_check, f"$.data[0]")
    ...
    def test_del_workload(self, args):
        ...
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        allure.dynamic.title(f"删除{payload_type}工作负载")
        ns = self.ns['name']
    ...
    def test_stop_and_start_workload(self, args):
        ...
        result = get_value_from_json(r_check, f"$.data[0]")
    ...
    def test_scale_deploy(self, args):
        ...
        allure.dynamic.title(f"{workload}实例伸缩")
    ...
    @allure.title("admin新建cronjob工作负载")
    def test_create_cronjob(self, args, paas_admin_login: PAASClient):
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ns = self.ns['name']
        r = paas_admin_login.cce_client.create_deployment_workload(
            payload_type, settings['CLUSTER_ID'], ns, yaml_str
        )
        check_status_code(r, 200)
        cnt = 0
        while cnt < 5:
            sleep(10)
            r_check = paas_admin_login.cce_client.get_deployment_by_ns(
                payload_type, settings['CLUSTER_ID'], ns
            )
        ...
        sleep(4)
        r = paas_admin_login.cce_client.del_deployment_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        sleep(3)
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['cronjob_workload'])
    @allure.title("admin删除cronjob工作负载")
    def test_delete_cronjob(self, args, paas_admin_login: PAASClient):
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['cronjob_workload'])
    @allure.title("admin停止、启动cronjob工作负载")
    def test_stop_cronjob(self, args, paas_admin_login: PAASClient):
        with allure.step("新建一个cronjob"):
            yaml_str = str(args['yaml'])
            deploy_name = args['yaml']['metadata']['name']
            payload_type = args['type']
            ...
            assert cnt < 5, "新建cronjob工作负载失败"
            sleep(4)
        r = paas_admin_login.cce_client.stop_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        check_status_code(r, 200)
        result = wait_action_finish_until_timeout(
            paas_admin_login.cce_client.get_deployment_by_ns,
        ...
        sleep(5)
        r = paas_admin_login.cce_client.start_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        check_status_code(r, 200)
        result = wait_action_finish_until_timeout(
