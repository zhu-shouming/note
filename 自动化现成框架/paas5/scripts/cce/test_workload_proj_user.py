...
class TestWorkload:
    ...
    def test_create_workload(self, args, paas_proj_user_login):
        allure.dynamic.title('普通用户' + args['title'])
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ns = self.ns['name']
        r = paas_proj_user_login.cce_client.create_deployment_workload(
        ...
        while cnt < 5:
            sleep(10)
            r_check = paas_proj_user_login.cce_client.get_deployment_by_ns(
        ...
        sleep(4)
        r = paas_proj_user_login.cce_client.del_deployment_workload(
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['create_deployment_workload'])
    def test_del_workload(self, args, paas_proj_user_login):
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        allure.dynamic.title('普通用户' + f"删除{payload_type}工作负载")
        ns = self.ns['name']
        new_workload(paas_proj_user_login, payload_type, deploy_name, yaml_str, ns)
        ...
        while cnt < 15:
            ...
            result = get_value_from_json(r_check, f"$.data[?(@.name=='{deploy_name}')]")
            if not bool(result):
                break
            else:
                cnt = cnt + 1
    ...
    def test_stop_and_start_workload(self, args, paas_proj_user_login):
        allure.dynamic.title('普通用户' + args['title'])
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ...
        action = paas_proj_user_login.cce_client.stop_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        ...
        sleep(3)
        action = paas_proj_user_login.cce_client.start_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        check_status_code(action, 200)
        cnt = 0
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['create_deployment_workload'])
    def test_scale_deploy(self, args, paas_proj_user_login: PAASClient):
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ns = self.ns['name']
        ...
        allure.dynamic.title('普通用户' + f"{workload}实例伸缩")
        ...
        ns = self.ns['name']
        r = paas_proj_user_login.cce_client.scale_workload(
    ...
    @allure.title("普通用户新建job")
    def test_create_job_workload(self, args, paas_proj_user_login):
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ns = self.ns['name']
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['job_workload'])
    @allure.title("普通用户删除job")
    def test_delete_job_workload(self, args, paas_proj_user_login):
    ...
    @allure.title("普通用户新建cronjob工作负载")
    def test_create_cronjob(self, args, paas_proj_user_login: PAASClient):
        yaml_str = str(args['yaml'])
        deploy_name = args['yaml']['metadata']['name']
        payload_type = args['type']
        ns = self.ns['name']
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['cronjob_workload'])
    @allure.title("普通用户删除cronjob工作负载")
    def test_delete_cronjob(self, args, paas_proj_user_login: PAASClient):
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize("args", data['cronjob_workload'])
    @allure.title("普通用户停止、启动cronjob工作负载")
    def test_stop_cronjob(self, args, paas_proj_user_login: PAASClient):
        with allure.step("新建一个cronjob"):
            yaml_str = str(args['yaml'])
            deploy_name = args['yaml']['metadata']['name']
            payload_type = args['type']
            ...
            assert cnt < 5, "新建cronjob工作负载失败"
            sleep(4)
        r = paas_proj_user_login.cce_client.stop_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        check_status_code(r, 200)
        result = wait_action_finish_until_timeout(
            paas_proj_user_login.cce_client.get_deployment_by_ns,
        ...
        sleep(5)
        r = paas_proj_user_login.cce_client.start_workload(
            payload_type, settings['CLUSTER_ID'], ns, deploy_name
        )
        check_status_code(r, 200)
        result = wait_action_finish_until_timeout(
