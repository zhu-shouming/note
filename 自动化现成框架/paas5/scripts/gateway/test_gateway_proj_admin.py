...
class TestGateway:
    ...
    def test_create_gateway(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("1.填写信息"):
            resource = args['container_spec']
            kong_ip = get_worker_node(paas_proj_admin_login, 0)
            ...
            loop = 3
            while loop:
                http_port = generate_kong_port(paas_proj_admin_login, kong_ip, cluster_name=cluster_name)
                https_port = generate_kong_port(paas_proj_admin_login, kong_ip, cluster_name=cluster_name)
        ...
        with allure.step("2.创建服务网关"):
            create_gateway(paas_proj_admin_login, gw_name, kong_ip, http_port, https_port, resource, cluster_name,
                           data_volume, storage_class=storage_class)
        with allure.step("3.校验状态"):
            exp_status = "RUNNING"
            exp_path = "$.data[0].status"
            check_status_timeout(exp_status, exp_path, get_gw_by_name, 1800, paas_client=paas_proj_admin_login, name=gw_name)
        with allure.step("4.删除服务网关"):
            delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_gateway'])
    def test_restart_gateway(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 重启服务网关")
        with allure.step("SETUP：创建服务网关"):
            ...
            storage_class = args['storage_class']
            data_volume = args['data_volume']
            setup_create_gateway(paas_proj_admin_login, gw_name, resource, kong_ip, storage_class, data_volume)
        with allure.step("重启服务网关"):
            restart_gateway(paas_proj_admin_login, gw_name)
        with allure.step("TEARDOWN: 删除服务网关"):
            delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_gateway'])
    def test_resize_gateway(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 扩缩容服务网关容器规格")
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            ...
            storage_class = args['storage_class']
            data_volume = args['data_volume']
            setup_create_gateway(paas_proj_admin_login, gw_name, old_resource, kong_ip, storage_class, data_volume)
        with allure.step("1.扩容服务网关"):
            ...
            resize_gateway(paas_proj_admin_login, gw_name, new_resource)
        with allure.step("2.缩容服务网关"):
            resize_gateway(paas_proj_admin_login, gw_name, old_resource)
        with allure.step("TEARDOWN: 删除服务网关"):
            delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_gateway'])
    def test_replicas_gateway(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 扩缩容服务网关容器实例")
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            resource = args['container_spec']
            kong_ip = get_worker_node(paas_proj_admin_login, 0)
            ...
            data_volume = args['data_volume']
            setup_create_gateway(paas_proj_admin_login, gw_name, resource, kong_ip, storage_class, data_volume)
            gw_info = get_gw_by_name(paas_proj_admin_login, gw_name)['data'][0]
            origin_kong_ip = gw_info['kongIp']
            origin_http_port = gw_info['kongHttpProxyPort']
            origin_https_port = gw_info['kongHttpsProxyPort']
        with allure.step("1.扩容服务网关"):
            replicas_kong_ip = get_worker_node(paas_proj_admin_login, 1)
            ...
            replicas_gateway(paas_proj_admin_login, gw_name, exp_business_ips, exp_http_ports, exp_https_ports)
        with allure.step("2.缩容服务网关"):
            replicas_gateway(paas_proj_admin_login, gw_name, origin_kong_ip, origin_http_port, origin_https_port)
        with allure.step("TEARDOWN: 删除服务网关"):
            delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_gateway'])
    def test_create_api_group(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 创建接口分组")
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            resource = args['container_spec']
            kong_ip = get_worker_node(paas_proj_admin_login, 0)
        ...
        with allure.step("创建接口分组"):
            ...
            create_api_group(paas_proj_admin_login, gw_name, api_group_name)
        with allure.step("TEARDOWN"):
            with allure.step("1.删除接口分组"):
                delete_api_group(paas_proj_admin_login, gw_name, api_group_name)
    ...
    def test_create_api(self, paas_proj_admin_login, args1, args):
        show_testcase_title("项目管理员 -- 创建接口")
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
        ...
        with allure.step("发布接口"):
            ...
            create_apis(paas_proj_admin_login, gw_name, api_group_name, api_name, svc_params, kong_params)
        with allure.step("验证接口功能"):
            sleep(10)
            api_detail = get_apis_detail_by_name(paas_proj_admin_login, gw_name, api_name)
        ...
        with allure.step("TEARDOWN"):
            with allure.step("1.删除接口"):
                delete_api_by_name(paas_proj_admin_login, gw_name, api_name)
    ...
    def test_create_api_versions(self, paas_proj_admin_login, args1, args):
        show_testcase_title("项目管理员 -- 创建接口版本")
        ...
        with allure.step("创建版本"):
            ...
            create_apis_version(paas_proj_admin_login, gw_name, api_name, version, service_list, items_header=items_header)
    ...
    def test_offline_online_apis(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 下线/上线接口")
        with allure.step("SETUP"):
            with allure.step("1.创建服务网关"):
                gw_name = "auto" + get_random_string(5, char_type=0)
                resource = args['container_spec']
        ...
        with allure.step("下线接口"):
            offline_apis_by_name(paas_proj_admin_login, gw_name, api_name)
        with allure.step("上线接口"):
            online_apis_by_name(paas_proj_admin_login, gw_name, api_name, svc_params['app_message'])
        with allure.step("TEARDOWN"):
            with allure.step("1.删除接口"):
                delete_api_by_name(paas_proj_admin_login, gw_name, api_name)
            with allure.step("2.删除接口分组"):
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_gateway'])
    def test_create_proxy_cache(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 创建代理缓存策略")
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            resource = args['container_spec']
            kong_ip = get_worker_node(paas_proj_admin_login, 0)
        ...
        with allure.step("创建代理缓存策略"):
            ...
            create_proxy_cache(paas_proj_admin_login, gw_name, policy_name)
        with allure.step("TEARDOWN"):
            with allure.step("1.删除代理缓存策略"):
                delete_proxy_cache(paas_proj_admin_login, gw_name, policy_name)
            with allure.step("2.删除网关"):
                delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_apis'])
    def test_proxy_cache_bind_and_unbind_api(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 代理缓存策略绑定/解绑网关")
        with allure.step("SETUP"):
            with allure.step("1.创建服务网关"):
                gw_name = "auto" + get_random_string(5, char_type=0)
                resource = args['container_spec']
        ...
        with allure.step("1.代理缓存策略绑定接口"):
            bind_apis_with_proxy_cache(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
        ...
        with allure.step("3.代理缓存策略解绑接口"):
            unbind_apis_with_proxy_cache(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
    ...
    def test_create_flow_control(self, paas_proj_admin_login, args1, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            resource = args1['container_spec']
        ...
        with allure.step("创建流控策略"):
            ...
            create_flow_control(paas_proj_admin_login, gw_name, policy_name, limit_by, per_sec, per_min, per_hour,
                                per_day)
        with allure.step("TEARDOWN"):
            with allure.step("1.删除流控策略"):
                delete_flow_control(paas_proj_admin_login, gw_name, policy_name)
            with allure.step("2.删除网关"):
                delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_apis'])
    def test_flow_control_bind_and_unbind_api(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 流控策略（按接口）绑定/解绑接口")
        with allure.step("SETUP"):
            with allure.step("1.创建服务网关"):
                gw_name = "auto" + get_random_string(5, char_type=0)
                resource = args['container_spec']
        ...
        with allure.step("1.流控策略绑定接口"):
            bind_apis_with_flow_control(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
        with allure.step("2.校验绑定接口"):
            sleep(10)
            api_detail = get_apis_detail_by_name(paas_proj_admin_login, gw_name, api_name)
            kong_url = api_detail['data']['kongUrl'][0]
        ...
        with allure.step("3.代理缓存策略解绑接口"):
            unbind_apis_with_flow_control(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
        with allure.step("4.校验解绑接口"):
            sleep(10)
            api_detail = get_apis_detail_by_name(paas_proj_admin_login, gw_name, api_name)
            kong_url = api_detail['data']['kongUrl'][0]
    ...
    def test_create_access_control(self, paas_proj_admin_login, args1, args):
        with allure.step("SETUP：创建服务网关"):
            gw_name = "auto" + get_random_string(5, char_type=0)
            resource = args1['container_spec']
            kong_ip = get_worker_node(paas_proj_admin_login, 0)
        ...
        with allure.step("创建安全控制策略"):
            ...
            create_access_control(paas_proj_admin_login, gw_name, policy_name, limit_type, config=config)
        with allure.step("TEARDOWN"):
            with allure.step("1.删除安全控制策略"):
                delete_access_control(paas_proj_admin_login, gw_name, policy_name)
            with allure.step("2.删除网关"):
                delete_gateway(paas_proj_admin_login, gw_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_apis'])
    def test_access_control_bind_and_unbind_api(self, paas_proj_admin_login, args):
        show_testcase_title("项目管理员 -- 安全控制策略绑定/解绑接口")
        with allure.step("SETUP"):
            with allure.step("1.创建服务网关"):
                gw_name = "auto" + get_random_string(5, char_type=0)
                resource = args['container_spec']
        ...
        with allure.step("1.安全控制策略绑定接口"):
            bind_apis_with_access_control(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
        with allure.step("2.校验绑定接口"):
            sleep(5)
            api_detail = get_apis_detail_by_name(paas_proj_admin_login, gw_name, api_name)
            kong_url = api_detail['data']['kongUrl'][0]
        ...
        with allure.step("3.安全控制策略解绑接口"):
            unbind_apis_with_access_control(paas_proj_admin_login, gw_name, policy_name, api_name, api_group_name)
        with allure.step("4.校验解绑接口"):
            sleep(5)
            api_detail = get_apis_detail_by_name(paas_proj_admin_login, gw_name, api_name)
            kong_url = api_detail['data']['kongUrl'][0]
