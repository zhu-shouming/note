...
if platform.system() == "Windows":
    dataPath = os.path.join(cur_path, 'data_proj_user_win.yml')
else:
    dataPath = os.path.join(cur_path, 'data_proj_user_linux.yml')
...
class TestDeploy:
    ...
    def test_upload_package(self, paas_proj_user_login, args):
        ...
        with allure.step("2.上传应用包到私有仓库"):
            upload_package(paas_proj_user_login, package_name, package_version, tag_name, package_type, file_path,
                           repository)
        with allure.step("TEARDOWN：删除应用包"):
            delete_package(paas_proj_user_login, package_name, [package_version], repository)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['test_apply_for_publishing_package_to_public'])
    def test_apply_for_publishing_package_to_public(self, paas_admin_login, paas_proj_admin_login,
                                                    paas_proj_user_login, args):
        show_testcase_title(args['title'])
        ...
        with allure.step("2.申请发布应用包到公有仓库"):
            apply_for_publishing_package(paas_proj_user_login, package_name, package_version)
        ...
        with allure.step("TEARDOWN"):
            ...
            delete_package(paas_proj_user_login, package_name, [package_version], visible=False)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['test_delete_package_with_more_visions'])
    def test_delete_package_with_more_visions(self, paas_proj_user_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 上传应用包"):
            ...
            # 查看应用包是否上传成功
            package_info = get_package_by_name(paas_proj_user_login, package_name, visible=repository)
        ...
        with allure.step("TEARDOWN：删除应用包"):
            delete_package(paas_proj_user_login, package_name, package_versions, repository)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_upload_package'])
    @allure.description("下载应用包")
    def test_download_package(self, paas_proj_user_login, args):
        show_testcase_title("普通用户 -- 下载" + args['title'])
        with allure.step("SETUP: 上传应用包"):
            tag_name = settings['TAG']
            package_version = get_random_string(5, char_type=3)
            package_type = args['package_type']
        ...
        with allure.step("下载应用包"):
            downloaded_file = download_package(paas_proj_user_login, package_name, package_version, repository)
    ...
    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['test_apply_for_deploying_package_of_war'])
    @allure.description("从应用仓库部署war包应用")
    def test_apply_for_deploying_package_of_war(self, paas_proj_admin_login, paas_proj_user_login, args):
        show_testcase_title(args['title'])
        with allure.step("1.填写上传应用包信息"):
            # 应用包名称只能小写，否则部署不成功
            package_name = "auto" + get_random_string(6, char_type=0).lower()
            package_version = get_random_string(5, char_type=3)
            package_type = args['package_type']
        ...
        with allure.step("3.创建应用组并部署应用"):
            ...
            resource_type = "container" if args['resource_type'] == "容器集群" else "container"
        ...
        with allure.step("TEARDOWN: 1.删除应用组"):
            ...
            check_del_result_timeout(get_app_group_by_name, paas_client=paas_proj_user_login, app_group_name=app_group_name)
