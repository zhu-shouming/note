...
dataPath = os.path.join(cur_path, 'data_proj_admin.yml')
...
class TestAppMgmt:
    ...
    def test_create_app_group(self, paas_proj_admin_login, args):
        ...
        if engine_type == "Spring Cloud":
            engine_info = new_spingcloud_engine(paas_proj_admin_login)
            engine_name = engine_info['name']
            engine_id = engine_info['id']
        with allure.step("1.创建应用组"):
            create_app_group(paas_proj_admin_login, app_group_name, engine_type=engine_type, svc_engine_id=engine_id)
            exp_status = "running"
            exp_path = "$.data[0].status"
            check_status_timeout(exp_status, exp_path, get_app_group_by_name, paas_client=paas_proj_admin_login,
                                 app_group_name=app_group_name)
        with allure.step("TEARDOWN: 删除应用组和微服务引擎"):
            delete_app_group_by_name(paas_proj_admin_login, app_group_name)
            if engine_type == "Spring Cloud":
                delete_micro_engine(paas_proj_admin_login, engine_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['test_create_app_group'])
    @allure.description("新建基于SpringCloud/ Istio/ 自定义微服务引擎的带有ns的应用组")
    def test_create_app_group_with_ns(self, paas_proj_admin_login, args):
        ...
        if engine_type == "Spring Cloud":
            ...
            engine_id = engine_info['id']
        with allure.step("1.创建应用组"):
            create_app_group(paas_proj_admin_login, app_group_name, namespace=namespace, engine_type=engine_type,
    ...
    def test_action_app_group_with_war(self, paas_proj_admin_login, args1, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组并部署应用且应用正常对外提供服务"):
            ...
            # 部署到私有仓库
            app_info = setup_create_war_app(paas_proj_admin_login, cluster_name, package_name, package_version,
                                            container_spec, svc_access_control, app_svc_info, repository=False,
                                            engine_type="others")
        with allure.step("2.停止应用组"):
            app_name = app_info['data'][0]['name']
            app_group_name = app_info['data'][0]['app_group_name']
            action_stop = args['action_stop']
            status_running = "running"
            exp_stopped = "stopped"
            action_app_group(paas_proj_admin_login, app_group_name, action_stop, status_running)
            exp_path = "$.data[0].status"
            # 应用状态变化
            check_status_timeout("Stopped", exp_path, get_app_by_name, paas_client=paas_proj_admin_login,
                                 app_name=app_name, app_group_name=app_group_name)
            check_status_timeout(exp_stopped, exp_path, get_app_group_by_name, paas_client=paas_proj_admin_login,
                                 app_group_name=app_group_name)
        with allure.step("3.启动应用组"):
            action_start = args['action_start']
            status_stopped = "stopped"
            exp_running = "running"
            action_app_group(paas_proj_admin_login, app_group_name, action_start, status_stopped)
            exp_path = "$.data[0].status"
            check_status_timeout("OK", exp_path, get_app_by_name, paas_client=paas_proj_admin_login, app_name=app_name,
                                 app_group_name=app_group_name)
            check_status_timeout(exp_running, exp_path, get_app_group_by_name, paas_client=paas_proj_admin_login,
                                 app_group_name=app_group_name)
    ...
    def test_create_jar_app_with_spring_cloud(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建基于Spring Cloud微服务引擎的应用组"):
            ...
            app_group_info = setup_create_app_group(paas_proj_admin_login)
        ...
        with allure.step("2.创建应用"):
            create_jar_app(paas_proj_admin_login, app_name, app_version, app_group_name, deploy_type, resource_type,
        ...
        with allure.step("4.验证应用正常对外提供服务"):
            ...
            check_app_svc_status(paas_proj_admin_login, app_name, app_svc_info, app_group_name=app_group_name)
        with allure.step("TEARDOWN: 清理资源"):
    ...
    def test_action_app_with_image(self, paas_proj_admin_login, args1, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组并部署应用且应用正常对外提供服务"):
            cluster_name = settings['CLUSTER_NAME']
            ...
            svc_access_control = args1['svc_access_control']
            app_svc_info = args1['app_svc_info']
            app_info_init = setup_create_image_app(paas_proj_admin_login, cluster_name, package_name, package_version,
        ...
        with allure.step("1.重启应用"):
            ...
            current_status = "OK"
            exp_status = "OK"
            action_app_info = action_app(paas_proj_admin_login, app_name, action_restart, current_status, app_group_name)
        ...
        with allure.step("2.停止应用"):
            ...
            current_status = "OK"
            exp_status = "Stopped"
            action_app_info = action_app(paas_proj_admin_login, app_name, action_stop, current_status, app_group_name)
        ...
        with allure.step("3.启动应用"):
            ...
            exp_status = "OK"
            action_app_info = action_app(paas_proj_admin_login, app_name, action_start, current_status, app_group_name)
    ...
    def test_upgrade_and_rollback_jar_app(self, paas_proj_admin_login, args1, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组并部署应用且应用正常对外提供服务"):
            cluster_name = settings['CLUSTER_NAME']
            ...
            app_info_init = setup_create_jar_app(paas_proj_admin_login, cluster_name, package_name, package_version,
                                                 container_spec, svc_access_control, old_app_svc_info,
                                                 repository=False, engine_type="others")
        with allure.step("1.升级应用"):
            ...
            upgrade_jar_app(paas_proj_admin_login, app_name, app_package_info)
        with allure.step("2.检查应用是否升级成功"):
            new_app_svc_info = args['app_svc_info']
            check_app_svc_status(paas_proj_admin_login, app_name, new_app_svc_info)
        with allure.step("3.回滚应用"):
            rollback_jar_app(paas_proj_admin_login, app_name)
        with allure.step("2.检查应用是否回滚成功"):
            old_app_svc_info = {
                "app_uri": args1['app_svc_info']['app_uri'],
                "app_message": args1['app_svc_info']['app_message']
            }
            check_app_svc_status(paas_proj_admin_login, app_name, old_app_svc_info)
        with allure.step("TEARDOWN: 删除应用组"):
            delete_app_group_by_name(paas_proj_admin_login, app_group_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['test_create_war_app'])
    def test_create_war_app(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组"):
            package_name = settings['PROJ_ADMIN_PACKAGE_NAME_OF_WAR']
            package_version = settings['PROJ_ADMIN_PACKAGE_VERSION_OF_WAR']
            app_group_info = setup_create_app_group(paas_proj_admin_login, engine_type="others")
            app_group_name = app_group_info['name']
        with allure.step("1.填写应用信息"):
            with allure.step("1-1.创建应用第一步"):
                app_name = get_random_string(5).lower()
            ...
            with allure.step("1-2.创建应用第二步"):
                ...
                if 'svc_access_control' in args.keys():
                    ...
                    svc_access_control_info.update(node_port_info)
        with allure.step("2.创建应用"):
            create_war_app(paas_proj_admin_login, app_name, app_version, app_group_name, deploy_type, resource_type,
                           app_type_info, cluster_info, app_package_info, container_spec, tomcat_version, jdk_version,
                           svc_access_control_info=svc_access_control_info)
        with allure.step("3.校验应用创建结果"):
            exp_status = "OK"
            exp_path = "$.data[0].status"
            check_status_timeout(exp_status, exp_path, get_app_by_name, paas_client=paas_proj_admin_login,
                                 app_name=app_name, app_group_name=app_group_name)
        with allure.step("4.验证应用正常对外提供服务"):
            app_svc_info = {
                "app_uri": args['app_svc_info']['app_uri'],
    ...
    def test_create_front_app(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组"):
        ...
        with allure.step("1.填写应用信息"):
            ...
            with allure.step("1-2.创建应用第二步"):
                ...
                if 'svc_access_control' in args.keys():
                    ...
                    svc_access_control_info.update(node_port_info)
        with allure.step("2.创建应用"):
            create_front_app(paas_proj_admin_login, app_name, app_version, app_group_name, deploy_type, resource_type,
    ...
    def test_create_helm_app(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组"):
        ...
        with allure.step("2.创建应用"):
            create_helm_app(paas_proj_admin_login, app_name_prefix, app_version, app_group_name, resource_type,
    ...
    def test_create_image_app(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建应用组"):
            package_name = settings['PACKAGE_NAME_OF_IMAGE']
            package_version = settings['PACKAGE_VERSION_OF_IMAGE']
            app_group_info = setup_create_app_group(paas_proj_admin_login, engine_type="Istio")
            app_group_name = app_group_info['name']
        with allure.step("1.填写应用信息"):
            with allure.step("1-1.创建应用第一步"):
                app_name = get_random_string(5).lower()
            ...
            with allure.step("1-2.创建应用第二步"):
                ...
                if 'svc_access_control' in args.keys():
                    ...
                    svc_access_control_info.update(node_port_info)
        with allure.step("2.创建应用"):
            create_image_app(paas_proj_admin_login, app_name, app_version, app_group_name, deploy_type, resource_type,
    ...
    def test_delete_apps_patch(self, paas_proj_admin_login, args1, args):
        show_testcase_title(args['title'])
        with allure.step("SETUP: 创建war包和前端包应用"):
            ...
            svc_access_control = args1['svc_access_control']
            app_svc_info = args1['app_svc_info']
            apps_info_init = setup_create_war_and_front_app(paas_proj_admin_login, cluster_name, package_info,
                                                            container_spec, svc_access_control, app_svc_info,
                                                            repository=False, engine_type="others")
            app_group_name = apps_info_init['data'][0]['app_group_name']
            allure.attach(f"{app_group_name}")
        with allure.step("批量删除应用"):
            ...
            delete_apps_patch_by_names(paas_proj_admin_login, app_names, delete_pvc)
        with allure.step("TEARDOWN: 删除应用组"):
            delete_app_group_by_name(paas_proj_admin_login, app_group_name)

    @pytest.mark.L5
    @pytest.mark.parametrize('args', data_file['setup_create_front_app'])
    @allure.title("项目管理员 -- 弹性伸缩应用")
    def test_scale_front_app(self, paas_proj_admin_login, paas_admin_login, args):
        with allure.step("SETUP"):
            with allure.step("1.创建应用组并部署应用且应用正常对外提供服务"):
                ...
                app_info_init = setup_create_front_app(paas_proj_admin_login, cluster_name, package_name,
                                                       package_version, container_spec, svc_access_control, app_svc_info,
                                                       repository=False, engine_type="others")
        ...
        with allure.step("2.扩容"):   # 扩容规格= current*2，组装扩容请求
            ...
            scale_app(paas_proj_admin_login, app_name, new_resources)
        with allure.step("3.缩容"):
            scale_app(paas_proj_admin_login, app_name, resources)
        with allure.step("TEARDOWN: 删除应用组"):
            delete_app_group_by_name(paas_proj_admin_login, app_group_name)
    ...
    def test_create_jar_app_with_storage(self, paas_proj_admin_login, args):
        show_testcase_title(args['title'])
    ...
    # def test_create_jar_app_with_spring_cloud_on_mcp(self, paas_proj_admin_login, args):
    #     show_testcase_title(args['title'])
    #     with allure.step("SETUP: 创建基于Spring Cloud微服务引擎的应用组"):
    #         package_name = settings['PROJ_ADMIN_PACKAGE_NAME_OF_JAR']
    #         package_version = settings['PROJ_ADMIN_PACKAGE_VERSION_OF_JAR']
    ...
    #             svc_access_control_info.update(node_port_info)
    #         # 查询边缘节点
    #         mcp_plaintext_list = get_mcp_nodes_list(paas_proj_admin_login, settings['MCP_CLUSTER_NAME'])
    #         # 创建应用
    #         create_jar_app(paas_proj_admin_login, app_name, app_version, app_group_name, deploy_type, resource_type,
    #                        app_type_info, cluster_info, app_package_info, container_spec, jdk_version,
    ...
    # def test_create_war_app_on_mcp(self, paas_proj_admin_login, args):
    #     show_testcase_title(args['title'])
    #     with allure.step("SETUP: 创建应用组"):
    #         package_name = settings['PROJ_ADMIN_PACKAGE_NAME_OF_WAR']
    #         package_version = settings['PROJ_ADMIN_PACKAGE_VERSION_OF_WAR']
    ...
    # def test_create_front_app_on_mcp(self, paas_proj_admin_login, args):
    #     show_testcase_title(args['title'])
    #     with allure.step("SETUP: 创建应用组"):
    #         package_name = settings['PROJ_ADMIN_PACKAGE_NAME_OF_WEB']
    #         package_version = settings['PROJ_ADMIN_PACKAGE_VERSION_OF_WEB']
    ...
    # def test_create_image_app_on_mcp(self, paas_proj_admin_login, args):
    #     show_testcase_title(args['title'])
    #     with allure.step("SETUP: 创建应用组"):
    #         package_name = settings['PACKAGE_NAME_OF_IMAGE']
    #         package_version = settings['PACKAGE_VERSION_OF_IMAGE']
