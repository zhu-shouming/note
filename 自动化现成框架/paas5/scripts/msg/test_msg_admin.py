...
class TestMicroEngine:
    ...
    def test_create_micro_engine_with_springcloud(self, paas_admin_login: PAASClient, args):
        allure.dynamic.title(args["title"])
        cluster_name = settings["CLUSTER_NAME"]
        version = args["version"]
        engine_name = "auto" + get_random_string(4)
        org_id = paas_admin_login.login_info.default_project_id
        midware = []
        ...
        with allure.step("指定集群"):
            response = paas_admin_login.cce_client.get_cce_list(name=cluster_name)
            check_status_code(response)
            cluster = get_value_from_json(response, f"$.data[?(@.name=='{cluster_name}')]")
            assert cluster, f"找不到指定的集群：{cluster_name}"
            cluster_id = cluster["uuid"]
        with allure.step("配置引擎类型及版本信息"):
            response = paas_admin_login.msg_client.get_msg_sku_list()
            check_status_code(response, 200)
            spingcloud_msg = get_value_from_json(response, f"$.data[?(@.version=='{version}')]")
        ...
        with allure.step("配置注册中心"):
            ...
            if args["discover"]["component"] == "nacos":
                r = paas_admin_login.msg_client.get_msg_sku_list()
                ...
                response = paas_admin_login.msg_client.get_discovery_info(version)
                check_status_code(response, 200)
        ...
        if "config" in args:
            with allure.step("设置配置中心"):
                ...
                response = paas_admin_login.msg_client.get_config_info(version)
                check_status_code(response, 200)
        ...
        if "gateway" in args:
            with allure.step("设置API网关和缓存信息"):
                ...
                response = paas_admin_login.msg_client.get_gateway_info(version)
                check_status_code(response, 200)
        ...
        with allure.step("创建微服务引擎"):
            response = paas_admin_login.msg_client.create_micro_engine_with_springcloud(
                engine_name,
                org_id,
                cluster_name,
                cluster_id,
        ...
        with allure.step("校验微服务引擎状态"):
            cnt = 0
            while cnt < 30:
                engine = get_micro_engine_by_name(paas_admin_login, engine_name)
                if bool(engine):
    ...
    def test_delete_micro_engine(self, paas_admin_login):
        with allure.step("新建微服务引擎"):
            name = "auto" + get_random_string(4)
            new_spingcloud_engine(paas_admin_login, name)
        with allure.step("删除微服务引擎"):
            time.sleep(3)
            delete_micro_engine(paas_admin_login, name)

    @pytest.mark.L5
    @pytest.mark.Smoke
    @allure.title("查询微服务引擎列表")
    def test_get_micro_engines_list(self, paas_admin_login: PAASClient):
        with allure.step("查询微服务引擎列表"):
            org_id = paas_admin_login.login_info.default_project_id
            micro_engine_list_response = paas_admin_login.msg_client.query_micro_engine(
    ...
    def test_create_micro_engine_with_dubbo(self, paas_admin_login: PAASClient, args):
        allure.dynamic.title(args["title"])
        ...
        create_msg_response = (
            paas_admin_login.msg_client.create_micro_engine_with_dubbo(
                org_id,
                msg_name,
                cluster_name,
                cluster_id,
        ...
        cnt = 0
        while cnt < 30:
            engine = get_micro_engine_by_name(paas_admin_login, msg_name)
            if bool(engine):
                if engine["state"] == "RUNNING":
                    break
            sleep(20)
        ...
        sleep(2)
        delete_micro_engine(paas_admin_login, msg_name)

    @pytest.mark.L5
    @pytest.mark.parametrize("args", data["modify_msg_info"])
    def test_moidfy_msg_info(self, paas_admin_login: PAASClient, args):
        allure.dynamic.title(args["title"])
        msg = new_spingcloud_engine(paas_admin_login)
        msg_id = msg["id"]
        name = "modify" + get_random_string(4).lower()
        engineIp = args["engineIp"]
        description = args["description"]
        cluser_id = settings["CLUSTER_ID"]
        res = paas_admin_login.msg_client.modify_msg_info(
    ...
    def test_add_component(self, paas_admin_login: PAASClient):
        name = "auto" + get_random_string(4).lower()
        mse_info = add_spring_mse(paas_admin_login, name)
        host = settings['HOST']
        if settings["CLUSTER_NAME"] != "default":
            midware_type = 1
        else:
        ...
        resp = paas_admin_login.msg_client.add_mse_compoent(mse_info['id'], data)
        check_status_code(resp, 200)
        cnt = 0
        while cnt < 30:
            time.sleep(20)
            r = paas_admin_login.msg_client.get_mse_component_status(
    ...
    def test_update_component_flavor(self, paas_admin_login: PAASClient, args):
        allure.dynamic.title(args['title'])
        name = "auto" + get_random_string(4).lower()
        mse_info = add_spring_mse(paas_admin_login, name)
        mse_id = mse_info['id']
        resp = paas_admin_login.msg_client.get_mse_component(mse_id)
        ...
        r = paas_admin_login.msg_client.update_mse_component_flavor(mse_id, data)
        check_status_code(r)
        cnt = 0
        while cnt < 30:
            time.sleep(20)
        ...
        check = paas_admin_login.msg_client.get_mse_component(mse_id)
...
class TestMse:
    ...
    @allure.title("跟新配置项")
    def test_update_configmap(self, paas_admin_login: PAASClient):
        with allure.step("新建配置"):
            mse_id = self.mse_id
            key = "k1"
            value = "v1"
            resp = paas_admin_login.msg_client.create_config(mse_id, key, value)
            check_status_code(resp)
            time.sleep(3)
            check = paas_admin_login.msg_client.get_mse_configmap(mse_id)
        ...
        with allure.step("更新配置"):
            new_value = "value new"
            resp = paas_admin_login.msg_client.update_configmap(
        ...
        with allure.step("校验修改是否成功"):
            ...
            paas_admin_login.msg_client.delete_configmap(mse_id, cm_id)

    @pytest.mark.L3
    @allure.title("新建配置项")
    def test_create_configmap(self, paas_admin_login: PAASClient):
        mse_id = self.mse_id
        key = "k1"
        value = "v1"
        resp = paas_admin_login.msg_client.create_config(mse_id, key, value)
    ...
    @allure.title("删除配置项")
    def test_del_configmap(self, paas_admin_login: PAASClient):
        mse_id = self.mse_id
        key = "k1"
        value = "v1"
        resp = paas_admin_login.msg_client.create_config(mse_id, key, value)
        ...
        cm_id = cm_info['id']
        time.sleep(3)
        del_r = paas_admin_login.msg_client.delete_configmap(mse_id, cm_id)
        check_status_code(del_r)
        time.sleep(3)
        check2 = paas_admin_login.msg_client.get_mse_configmap(mse_id)
