...
class TestCCR:
    ...
    def test_get_public_imgs(self, paas_proj_admin_login: PAASClient):
        res = paas_proj_admin_login.ccr_client.get_public_images()
        check_status_code(res, 200)
        data = res.json()
    ...
    @pytest.mark.L5
    @allure.title("查询镜像组织")
    def test_get_img_orgs(self, paas_proj_admin_login: PAASClient):
        res = paas_proj_admin_login.ccr_client.get_image_orgs()
        check_status_code(res, 200)

    @pytest.mark.L5
    @allure.title("查询特定镜像组织的所有镜像")
    def test_get_org_imgs(self, paas_proj_admin_login: PAASClient):
        with allure.step("从列表中选择一个镜像组织"):
            list_res = paas_proj_admin_login.ccr_client.get_image_orgs()
            check_status_code(list_res, 200)

            chose_org = get_value_from_json(list_res, "$.data.data[0].name")
        res = paas_proj_admin_login.ccr_client.get_imgs_by_imgProject(chose_org)
        check_status_code(res, 200)
    ...
    @pytest.mark.L5
    @allure.title("新建镜像组织")
    def test_creat_img_org(self, paas_proj_admin_login: PAASClient):
        with allure.step("新建镜像组织"):
            name = "auto" + get_random_string(4).lower()
            resp = paas_proj_admin_login.ccr_client.create_image_org(name)
            check_status_code(resp, 200)
            data = resp.json()
            assert data["status"] == True, f"请求响应消息不符合预期"
            assert data["msg"] == "ok", f"请求响应消息不符合预期"
        r = paas_proj_admin_login.ccr_client.get_image_orgs()
        check_status_code(r, 200)
        img_org_id = get_value_from_json(
            r, f"$.data.data[?(@.name=='{name}')].project_id"
        )
        sleep(4)
        paas_proj_admin_login.ccr_client.delete_image_org(img_org_id)

    @pytest.mark.L5
    @allure.title("删除镜像组织")
    def test_del_img_org(self, paas_proj_admin_login: PAASClient):
        name = "auto" + get_random_string(5).lower()
        check_name = paas_proj_admin_login.ccr_client.check_img_org_name(name)
        check_status_code(check_name, 200)
        msg = get_value_from_json(check_name, "$..msg")
        assert msg == "ok"
        res = paas_proj_admin_login.ccr_client.create_image_org(name)
        check_status_code(res, 200)
        sleep(5)
        check = paas_proj_admin_login.ccr_client.get_image_orgs(name=name)
        check_status_code(check, 200)
        status = get_value_from_json(check, "$.status")
        num = get_value_from_json(check, "$..total")
        assert status == True
        ...
        del_resp = paas_proj_admin_login.ccr_client.delete_image_org(img_org_id)
        check_status_code(del_resp, 200)

    @pytest.mark.L5
    @pytest.mark.parametrize("args", data["creat_access_token"])
    def test_create_access_token(self, paas_proj_admin_login: PAASClient, args):
        allure.dynamic.title(args["title"])
        name = "auto" + get_random_string(4).lower()
        res = paas_proj_admin_login.ccr_client.create_access_token(
        ...
        assert httpcode == 200
        time.sleep(3)
        res = paas_proj_admin_login.ccr_client.get_access_token(username=name)
        uuid = get_value_from_json(res, "$..uuid")
        harborId = get_value_from_json(res, "$..harborId")
        paas_proj_admin_login.ccr_client.delete_access_token(uuid, harborId)

    @pytest.mark.L5
    @pytest.mark.parametrize("args", data["creat_access_token"])
    @allure.title("删除用户访问凭证")
    def test_delete_access_token(self, paas_proj_admin_login: PAASClient, args):
        name = "auto" + get_random_string(4).lower()
        res = paas_proj_admin_login.ccr_client.create_access_token(
            name, args["fullname"], args["mail"], args["passwd"], args["comment"]
        )
        ...
        uuid = get_value_from_json(res, "$..uuid")
        harborId = get_value_from_json(res, "$..harborId")
        del_res = paas_proj_admin_login.ccr_client.delete_access_token(uuid, harborId)
        check_status_code(del_res, 200)
        del_check = paas_proj_admin_login.ccr_client.get_access_token(username=name)
    ...
    def test_reset_accessToken_passwd(self, paas_proj_admin_login: PAASClient, args):
        allure.dynamic.title(args['title'])
        ...
        with allure.step("重置密码"):
            res = paas_proj_admin_login.ccr_client.reset_accessToken_passwd(
    ...
    def test_upload_private_img(self, paas_proj_admin_login: PAASClient, args):
        allure.dynamic.title("组织管理员" + args['title'])
        private_img = settings['cce_image']
        description = args["description"]
        version = args["version"]
        user_id = paas_proj_admin_login.login_info.user_id
        tentantid = paas_proj_admin_login.login_info.default_project_id
        ...
        r = paas_proj_admin_login.ccr_client.upload_private_image(
        ...
        while cnt < 6:
            sleep(10)
            check_r = paas_proj_admin_login.ccr_client.get_imgs_by_imgProject(
        ...
        sleep(3)
        img_r = paas_proj_admin_login.ccr_client.get_imgs_by_imgProject(
            self.imgProject_name, img_name=img_name
        )
        ...
        r = paas_proj_admin_login.ccr_client.get_private_img_detail_info(
            ...
            "images": [
                {
                    ...
                    "imgUrl": img_url,
                    "tentantID": tentantid,
                    "userID": paas_proj_admin_login.login_info.user_id,
                }
            ],
            "tagCount": 1,
            "imgType": img_type,
        ...
        r_del = paas_proj_admin_login.ccr_client.delete_private_image(data)
        check_status_code(r_del, 200)
        cnt = 0
        while cnt < 5:
            sleep(6)
            r = paas_proj_admin_login.ccr_client.get_imgs_by_imgProject(
                self.imgProject_name, img_name=img_name
            )
    ...
    @pytest.mark.L5
    @allure.title("组织管理员删除私有镜像")
    @pytest.mark.parametrize('args', data['upload_private_img'])
    def test_del_private_img(self, paas_proj_admin_login: PAASClient, args):
        with allure.step("上传一个私有镜像"):
            ...
            resp = paas_proj_admin_login.ccr_client.upload_private_image(
    ...
    @pytest.mark.L5
    @allure.title("组织管理员申请发布私有镜像到公有仓库")
    @pytest.mark.parametrize('args', data['upload_private_img'])
    def test_public_private_img(
        self, paas_proj_admin_login: PAASClient, paas_admin_login: PAASClient, args
    ):
        ...
        with allure.step("申请发布镜像到公有仓库"):
            resp = paas_proj_admin_login.ccr_client.public_private_image(
        ...
        with allure.step("审批流程"):
            ...
            while cnt < 5:
                sleep(10)
                check = paas_proj_admin_login.ccr_client.get_public_images(
                    img_name=img_name
                )
        ...
        sleep(2)
        del_private_image_version(paas_proj_admin_login, self.imgProject_name, img_name)
