...
```python
# b64encode、b64decode来编码和解码字符串
>>> import base64
>>> content = 'Man is distinguished'
>>> base64.b64encode(content.encode())
b'TWFuIGlzIGRpc3Rpbmd1aXNoZWQ='
>>> content = b'TWFuIGlzIGRpc3Rpbmd1aXNoZWQ='
>>> base64.b64decode(content).decode()
'Man is distinguished'
# encode、decode用来编码和解码文件
```
...
由于`uuid4`存在概率型重复，那么在真正需要全局唯一标识符的地方最好不用使用它。在分布式环境下，`uuid1`是很好的选择，因为它能够保证生成ID的全局唯一性。下面是在**Python交互式环境中**使用`uuid1`函数生成全局唯一标识符的例子。或多个，多个`except`会按照书写的顺序依次匹配指定的异常，如果
